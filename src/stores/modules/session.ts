...
import { create_session, delete_session, get_session, get_session_list, update_session } from '@/api'
...
export const useSessionStore = defineStore('session', () => {
  ...
  const requestSessionList = async (page: number = currentPage.value, force: boolean = false) => {
    ...
    if (!force && ((page > 1 && !hasMore.value) || isLoading.value || isLoadingMore.value)) return
    ...
    try {
      ...
      if (!force) hasMore.value = (res?.length || 0) === pageSize.value
      if (!force) currentPage.value = page // 仅非强制刷新时更新页码
    } catch (error) {
  ...
  const loadMoreSessions = async () => {
    if (hasMore.value) await requestSessionList(currentPage.value + 1)
  }
  ...
  function processSessions(sessions: ChatSessionVo[]) {
    ...
    return sessions.map(session => {
      ...
      const diffDays = Math.floor((currentDate.getTime() - createDate.getTime()) / (1000 * 60 * 60 * 24))
